...
// Parse markdown back to NAReport structure
function parseMarkdownToReport(markdown: string): NAReport | null {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      } else if (line.startsWith('## Recommended Solution')) {
        currentSection = 'solution'
      } else if (line.startsWith('## Delivery Plan')) {
        currentSection = 'delivery_plan'
      ...
      if (currentSection === 'summary') {
        if (line.startsWith('**Problem Statement:**')) {
          ...
          currentSubSection = 'current_state'
        } else if (line.startsWith('**Root Causes:**')) {
          currentSubSection = 'root_causes'
        } else if (line.startsWith('**Objectives:**')) {
          currentSubSection = 'objectives'
      ...
      if (currentSection === 'solution') {
        if (line.startsWith('### Delivery Modalities')) {
          currentSubSection = 'modalities'
        } else if (line.startsWith('**Target Audiences:**')) {
          currentSubSection = 'audiences'
        } else if (line.startsWith('**Key Competencies:**')) {
          currentSubSection = 'competencies'
        } else if (line.startsWith('**Content Outline:**')) {
          currentSubSection = 'content_outline'
          ...
          } else if (currentSubSection === 'audiences') {
            report.solution.scope.audiences.push(item)
          } else if (currentSubSection === 'competencies') {
            report.solution.scope.competencies.push(item)
          } else if (currentSubSection === 'content_outline') {
            report.solution.scope.content_outline.push(item)
          }
        }
      }
      ...
      if (currentSection === 'delivery_plan') {
        if (line.startsWith('### Timeline')) {
          ...
          if (match) {
            const phase = {
              name: match[1],
              duration_weeks: parseInt(match[2]),
              goals: [] as string[],
              activities: [] as string[]
            }
            // Look ahead for goals and activities
            for (let j = i + 1; j < lines.length && j < i + 10; j++) {
              const nextLine = lines[j].trim()
              if (nextLine === 'Goals:') {
                for (let k = j + 1; k < lines.length && lines[k].startsWith('- '); k++) {
                  phase.goals.push(lines[k].substring(2).trim())
                }
              } else if (nextLine === 'Activities:') {
                for (let k = j + 1; k < lines.length && lines[k].startsWith('- '); k++) {
                  phase.activities.push(lines[k].substring(2).trim())
                }
              }
      ...
      if (currentSection === 'measurement') {
        ...
        } else if (line.startsWith('**Assessment Strategy:**')) {
          currentSubSection = 'assessment_strategy'
        } else if (line.startsWith('**Data Sources:**')) {
          currentSubSection = 'data_sources'
        } else if (line.startsWith('- ')) {
          const item = line.substring(2)
          if (currentSubSection === 'success_metrics') {
            report.measurement.success_metrics.push(item)
      ...
      if (currentSection === 'next_steps') {
        ...
        if (match) {
          report.next_steps.push(match[1])
        }
      }
      ...
      if (currentSection === 'risks' && line.startsWith('- **Risk:**')) {
        ...
        if (riskMatch && mitigationMatch) {
          report.risks.push({
            risk: riskMatch[1],
            mitigation: mitigationMatch[1]
          })
        }
      }
    }
