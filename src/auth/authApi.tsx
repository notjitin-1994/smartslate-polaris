...
export const authApi = {
  async login(params: { identifier: IdentifierValue; password: string }) {
    // Placeholder: integrate with backend later
    await new Promise((r) => setTimeout(r, 600))
    console.log('login', params)
  },
  async signup(params: { identifier: IdentifierValue; password: string }) {
    await new Promise((r) => setTimeout(r, 600))
    console.log('signup', params)
  },
  GoogleButton: function GoogleButton() {
    // Placeholder Google button - style aligned with app
    return (
      <button type="button" className="w-full inline-flex items-center justify-center gap-2 border border-slate-200 bg-white hover:bg-slate-50 text-slate-900 rounded-xl px-4 py-3 text-sm font-medium transition">
