...
export function SettingsPage() {
  // Nested under Portal header; keep title changes within parent only
  const navigate = useNavigate()
  const [user, setUser] = useState<User | null>(null)
  const [appearance, setAppearance] = useState<Appearance>('system')
  const fileInputRef = useRef<HTMLInputElement | null>(null)

  // Optional profile fields (all optional)
  ...
  async function onRemoveAvatar() {
    ...
    try {
      ...
      if (path) {
        // Best-effort deletion; ignore errors if policy disallows
  ...
  async function onDeleteAccount() {
    ...
    if (!ok) return
    try {
      // Best effort: mark for deletion for backend processing
      await getSupabase().auth.updateUser({ data: { delete_requested: true, delete_requested_at: new Date().toISOString() } })
    } catch {}
    // Attempt an Edge Function (if configured) to actually delete
    try {
  ...
  return (
      <div className="mx-auto max-w-4xl px-4 py-6 space-y-4 animate-fade-in-up">
