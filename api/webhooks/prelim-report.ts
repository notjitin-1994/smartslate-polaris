...
async function updateReportWithWebhookResult(
  reportTable: string,
  reportId: string,
  jobId: string,
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    // Check if the report exists and belongs to the job
    const { data: existingReport, error: fetchError } = await supabase
      .from(reportTable)
      .select('id, user_id, webhook_status, webhook_attempts')
    ...
    if (existingReport.webhook_status === 'success') {
      ...
      return res.status(200).json({ message: 'Webhook already processed successfully', report_id: payload.report_id })
    }

    // Update the report with the new research data
    const updatedReport = await updateReportWithWebhookResult(
      reportTable,
      payload.report_id,
      payload.job_id,
    ...
    console.log(`Successfully processed prelim-report webhook for job ${payload.job_id}, report ${payload.report_id}`)
    ...
    console.error(`Prelim-report webhook error for job ${payload.job_id}:`, err)
