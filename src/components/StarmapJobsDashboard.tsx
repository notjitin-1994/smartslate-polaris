...
export default function StarmapJobsDashboard() {
  ...
  async function loadJobs() {
    try {
      ...
      if (!error && data) {
        ...
        setSelectedIds((prev) => new Set(Array.from(prev).filter((id) => data.some((j) => j.id === id))))
  ...
  async function handleBulkDelete() {
    ...
    if (!confirm(`Delete ${selectedIds.size} starmap${selectedIds.size > 1 ? 's' : ''}? This cannot be undone.`)) return
    try {
      setBulkDeleting(true)
      for (const id of Array.from(selectedIds)) {
        setDeleting(id)
        const { error } = await deleteStarmapJob(id)
        if (!error) {
          ...
          <div className="flex-shrink-0 flex items-center gap-2">
            <label className="inline-flex items-center gap-2 text-white/80">
              {(() => {
                ...
                useEffect(() => {
                  ...
                  ref.current.indeterminate = selectedIds.size > 0 && selectedIds.size < jobs.length
                }, [selectedIds, jobs])
                return (
                  <input
                    ref={ref}
            ...
            {selectedIds.size > 0 && (
              <button
                ...
                className="px-3 py-2 rounded-lg bg-red-500/20 text-red-200 border border-red-500/30 hover:bg-red-500/30 disabled:opacity-60"
            ...
            <button
              ...
              className="px-4 py-2 rounded-lg transition-all bg-secondary-500/30 text-secondary-300 border border-secondary-400/50 hover:bg-secondary-500/40 hover:text-white"
            >
              <span className="inline-flex items-center gap-2">
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} aria-hidden="true">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
                </svg>
                <span>Begin new Discovery</span>
              </span>
            </button>
          </div>
        </div>
