...
export default function AreaChart({ spec }: { spec: any }) {
  ...
  const a11yLabel = spec?.a11y?.ariaLabel || spec?.title || 'Area chart'
  if (!RC) return <div className="h-64 rounded-xl border border-white/10 bg-white/5" aria-busy="true" />
  const { ResponsiveContainer, AreaChart: RCArea, Area, CartesianGrid, XAxis, YAxis, Tooltip, Legend } = RC
    ...
    <div className="glass-card p-4" role="figure" aria-label={a11yLabel}>
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <RCArea data={series} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
            ...
            <Legend />
            <Area type="monotone" dataKey="value" stroke="#60a5fa" fill="#60a5fa33" strokeWidth={2} />
          </RCArea>
        </ResponsiveContainer>
      </div>
    </div>
